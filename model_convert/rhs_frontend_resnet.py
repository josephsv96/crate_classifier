...
from keras.layers import Reshape, Activation, Conv2D, Input, MaxPooling2D, BatchNormalization, Flatten, Dense, Lambda, Add
...
class SpecialYOLO(object):
    ...
    def __init__(self, input_width,
        ...
        # stack 2
        # sub stack 1
        x, num_layer = SpecialYOLO.res_identity(x, 16, num_layer)
        # sub stack 2
        x, num_layer = SpecialYOLO.res_identity(x, 16, num_layer)
        ...
        # stack 3
        # sub stack 1
        x, num_layer = SpecialYOLO.res_conv(x, 16, num_layer)
        # sub stack 2
        x, num_layer = SpecialYOLO.res_identity(x, 16, num_layer)
        ...
        # stack 6
        x = Conv2D(self.num_classes, (1, 1), strides=(1, 1), padding='same',
                   name='conv_' + str(num_layer), use_bias=False)(x)
        x = BatchNormalization(name='norm_' + str(num_layer))(x)
        x = LeakyReLU(alpha=0.1)(x)
        ...
        x = Conv2D(self.num_classes, (1, 1), strides=(1, 1),
